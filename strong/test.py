...
def fourierExtrapolation(x, n_predict):
    ...
    tmp = fftpack.fft(x_notrend) 
    x_freqdom = fftpack.fft(x_notrend)[0:int(n/2)]  # detrended x in frequency domain
    tmp_f = fft.fftfreq(n)              # frequencies
    f = fft.fftfreq(n)[0:int(n/2)]              # frequencies
    ...
    restored_sig = np.ones(t.size) * np.absolute(x_freqdom[0])/n
    for i in indexes[1:1 + n_harm]:
        ampli = 2*np.absolute(x_freqdom[i]) / n   # amplitude
