...
ori <- readline("Enter Origin: ")
des <- readline("Enter Destination: ")
...
flight <- lapply(seq(1,length(flight)), function(x) flight[[x]][sort.list(flight$AVG_CNV)])
names(flight) <- str_name
# flight <- lapply(seq(1,length(flight)), function(x) flight[[x]][sort.list(as.character(flight$ADV_ARR_TIME))])
# names(flight) <- str_name
